...
disp(FAin*integral(@(X)function_1(X, DHin, Tin, Cpin, DCp, CAin, Tfref, TeqRef, DHStar, kfRef, E, KeqRef), 0, XTarget));
...
function yy = function_1(X, Dh0r, Tin, CpIn, DCp, CAin, Tfref, TeqRef, DHStar, kfRef, E, KeqRef)

    T = Tin + (-Dh0r*X)./(CpIn + DCp*X);
    kf  = kfRef*exp(-E/8.314*(1./T-1/Tfref));    % [1/h]
    kEq = KeqRef*exp(-(DHStar/8.314-DCp/8.314*TeqRef)*(1./T-1/TeqRef) + ...
          DCp/8.314*log(T/TeqRef));
    r  = kf.*CAin.*(1-(1+1./kEq).*X);
