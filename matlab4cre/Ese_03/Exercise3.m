...
MaxIter = 1000;
Teout_guessed = Tin+20;
threshold = 0.001;
alpha = 0.5;

for i = 1:MaxIter
    
    Yin = [FAin, FBin, Tin, Teout_guessed]';
    [z, Y] = ode45(@PFR, [0, L], Yin, []);
    Te = Y(:, 4);
    
    eps = Te(end) - Tein;
    if mod(i, 1) == 0
        fprintf('Iteration: %d, TGuess: %f, error: %f\n', i, Teout_guessed, eps)
    end 
    if (abs(eps) < threshold)
        break
    end
    
    Teout_guessed = Teout_guessed - alpha*eps;    
end
